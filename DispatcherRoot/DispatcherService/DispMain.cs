...
namespace Dispatcher
{
    public partial class DispMain : ServiceBase , IDisposable
    {
        ...
        void MyTcpManager3_TcpOpenEvent(object sender, EventArgs e)
        {
            ...
            MyTcpManager1.SetWait(false);

            MyTcpManager2.SetWait(false);
        ...
        private void allPortClose()
        {
            lock (lockObj)
            {
                ...
                mLog.WriteLog("TCP5000x", "allPortClose.");
                ...
                if (MyTcpManager1 != null)
                {
                    MyTcpManager1.SetWait(true);
                    MyTcpManager1.Close(false);
                    mLog.WriteLog("TCP5000x", "port1 closed.");
                }
                if (MyTcpManager2 != null)
                {
                    MyTcpManager2.SetWait(true);
                    MyTcpManager2.Close(false);
                    mLog.WriteLog("TCP5000x", "port2 closed.");
                }
                if (MyTcpManager3 != null)
                {
                    MyTcpManager3.Close(false);
                    mLog.WriteLog("TCP5000x", "port3 closed.");
                }
            }
        }
