...
namespace Dispatcher
{
    public class TcpManager : IDisposable
    {
        ...
        {
            IsWait = flag;
            OnException("Wait. flag=" + flag.ToString());
            disconnect_count = 0;
        }
            ...
            Task.Factory.StartNew(() =>
            {
                while (IsAlive)
                {
                    ...
                    if (TcpStatus == Status.Disconnect)
                    {
                        ...
                        {
                            OnException("State=[Disc] count="+ disconnect_count.ToString() + ":" + ex.ToString());
                            toDisconnect();
                        }
                    }
                    else if (TcpStatus == Status.Connect)
                    {
                        ...
                        bool FlagSend = true;
                        {
                            ...
                            {
                                OnException("State=[Cone] count=" + disconnect_count.ToString() + ":" + ex.ToString());
                                toDisconnect();
                            }
                        }
                    }
        ...
        {
            disconnect_count++;
            if (TcpStatus != Status.Disconnect || disconnect_count >= 10)
            {
                disconnect_count = 0;
                OnDisconnect();
            }
