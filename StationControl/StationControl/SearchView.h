...
public:
	CSearchView();           // 動的生成で使用される protected コンストラクター
	virtual ~CSearchView();
...
public:
	...
	BOOL m_bStation[CBarChild::mStationMax+1];
	BOOL m_bImStatus[CBarChild::mStatusFlagMax];
	CEdit m_edMessage;
	CSenderButton m_btSender[CBarChild::mStationMax+1];
	CColorCheckButton m_btStation[CBarChild::mStatusFlagMax+1];
...
inline CStationControlDoc* CSearchView::GetDocument() const
{
