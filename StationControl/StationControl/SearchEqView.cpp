...
#define GREAT_SEARCHEQ

// CSearchEqView

IMPLEMENT_DYNCREATE(CSearchEqView, CFormView)

CSearchEqView::CSearchEqView()
: CFormView(CSearchEqView::IDD)
, m_strText(_T(""))
{
	m_bOneStation = false;
	...
	for (int i = 0; i < CBarChild::mImStatusKindMax; i++)
		m_bImStatus[i] = TRUE;
	for (int i = 0; i < eEqHistStatus_Max; i++)
		m_bStatus[i] = TRUE;
...
static bool procEqSearchSort(const stSearchHistoryData& left, const stSearchHistoryData& right)
{
	CString str1, str2;
...
void CSearchEqView::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_SSTATUS5, m_bStatus[3]);
	DDX_Check(pDX, IDC_CHECK_SSTATUS6, m_bStatus[4]);
...
BEGIN_MESSAGE_MAP(CSearchEqView, CFormView)
	ON_BN_CLICKED(IDC_BUTTON_SELECTALL, &CSearchEqView::OnBnClickedButtonSelectall)
	ON_BN_CLICKED(IDC_BUTTON_EXEC, &CSearchEqView::OnBnClickedButtonExec)
	ON_BN_CLICKED(IDC_BUTTON_FILE, &CSearchEqView::OnBnClickedButtonFile)
	ON_BN_CLICKED(IDC_BUTTON_PRINT, &CSearchEqView::OnBnClickedButtonPrint)
	ON_WM_SIZE()
	//ON_WM_ERASEBKGND()
	ON_BN_CLICKED(IDC_BUTTON_NEXT, &CSearchEqView::OnBnClickedButtonNext)
	ON_BN_CLICKED(IDC_BUTTON_PREV, &CSearchEqView::OnBnClickedButtonPrev)
	ON_BN_CLICKED(IDC_CHECK_STATION1, &CSearchEqView::OnBnClickedCheckStation1)
	ON_BN_CLICKED(IDC_CHECK_STATION2, &CSearchEqView::OnBnClickedCheckStation2)
	ON_BN_CLICKED(IDC_CHECK_STATION3, &CSearchEqView::OnBnClickedCheckStation3)
	ON_BN_CLICKED(IDC_CHECK_STATION4, &CSearchEqView::OnBnClickedCheckStation4)
END_MESSAGE_MAP()
...
#ifdef _DEBUG
void CSearchEqView::AssertValid() const
{
	CFormView::AssertValid();
}

#ifndef _WIN32_WCE
void CSearchEqView::Dump(CDumpContext& dc) const
{
	CFormView::Dump(dc);
}
#endif
CStationControlDoc* CSearchEqView::GetDocument() const // デバッグ以外のバージョンはインラインです。
{
...
// CSearchEqView メッセージ ハンドラー


void CSearchEqView::OnInitialUpdate()
{
...
*/
/*============================================================================*/
void CSearchEqView::CreateHistroyList()
{
	// クライアント領域の取得
	CRect rectClient;
...
*/
/*============================================================================*/
void CSearchEqView::AutoSizeColumns(int col /*=-1*/)
{
...
*/
/*============================================================================*/
int CSearchEqView::AddItem(int nItem, int nSubItem, CString strItem, DWORD dwParam/*=0*/)
{
	LV_ITEM lv;
...
void CSearchEqView::OnBnClickedButtonSelectall()
{
	UpdateData(TRUE);
	for (int i = 0; i < CBarChild::mStationMax; i++)
		m_nSender[i] = 0;
...
void CSearchEqView::OnBnClickedButtonExec()
{
	UpdateData(TRUE);
	...
	m_SearchData.status = 0;
	for (int i = 0; i < eEqHistStatus_Max; i++){
		m_SearchData.status |= (m_bStatus[i] == TRUE) ? (1 << i) : (0 << i);
	}
	if (theApp.GetSelectStationMode() == eStation_1){
...
void CSearchEqView::OnBnClickedButtonFile()
{
	m_History.CsvOutput(m_strTitle);
}

void CSearchEqView::OnBnClickedButtonPrint()
{
...
*/
/*============================================================================*/
void CSearchEqView::SetDisplaySearch(int nFilterType, stFilterData& data)
{
	CWaitCursor wait;
...
SEARCH_END_LABEL:
	...
	mHistoryData.sort(procEqSearchSort);
	...
	for (itrh = mHistoryData.begin(); itrh != mHistoryData.end(); itrh++){
		for (int col = 0; col <= maxcol; col++){
			...
			case eSearchitem_Equipment:
				if ((*itrh).searchtype == eHistory_SearchEvent)
					str.Format(_T(""));
				else
...
*/
/*============================================================================*/
bool CSearchEqView::IsMessageEvent(TCHAR* pmsg)
{
...
bool CSearchEqView::_GetDate(stSearchHistoryData& hdata, CString cbuf)
{
	CString str, temp;
...
bool CSearchEqView::_GetTime(stSearchHistoryData& hdata, CString cbuf)
{
	CString str, temp;
...
bool CSearchEqView::_GetStation(stSearchHistoryData& hdata, CString cbuf)
{
	UINT nStation = theApp.GetSelectStation();
	hdata.station = nStation;
	return true;
...
bool CSearchEqView::_GetEquipment(stSearchHistoryData& hdata, CString cbuf)
{
	// 送信元
	typedef struct {
		CString		name;
...
bool CSearchEqView::_GetImStatus(stSearchHistoryData& hdata, CString cbuf)
{
	// 送信元
	typedef struct {
		CString		name;
...
bool CSearchEqView::_GetStatus(stSearchHistoryData& hdata, CString cbuf)
{
#ifndef GREAT_SEARCHEQ
	// 送信元
	typedef struct {
	...
	const stStatus mStatus[] = {
		...
		{ _T("MAINT/NORMAL"), eEqHistStatus_RemoteLocal },	// REMOTE/LOCALに含める
	...
	return false;
#else
	// 設備状態検索履歴の状態識別をリアルタイム履歴と同じにする
	switch (hdata.message % 10){
	case	1:/*POWER ON/OFF*/
		hdata.status = 4;
		break;
	case	2:/*REMOTE/LOCAL*/
		...
		break;
	case	3:/*MAINT/NORMAL*/
		hdata.status = 3;		// REMOTE/LOCALに含める
		break;
	case	4:/*WARNING*/
		hdata.status = 1;
		break;
...
bool CSearchEqView::_GetMessageID(stSearchHistoryData& hdata, CString cbuf)
{
	CString str, temp;
...
bool CSearchEqView::_GetMessageString(stSearchHistoryData& hdata, CString cbuf)
{
...
void CSearchEqView::OnSize(UINT nType, int cx, int cy)
{
...
BOOL CSearchEqView::OnEraseBkgnd(CDC* pDC)
{
	return CFormView::OnEraseBkgnd(pDC);
}


void CSearchEqView::OnBnClickedButtonNext()
{
	UpdateData(TRUE);
...
void CSearchEqView::OnBnClickedButtonPrev()
{
	UpdateData(TRUE);
	CString text = m_strText;
	text.MakeLower();
...
*/
/*============================================================================*/
bool CSearchEqView::WildCard(TCHAR* pwild, TCHAR* pstr)
{
	enum State {
		Exact,
		AnyRepeat,
...
void CSearchEqView::OnBnClickedCheckStation1()
{
	UpdateData(TRUE);
	GetDlgItem(IDC_BUTTON_SENDER)->EnableWindow(m_bStation[0]);
}
void CSearchEqView::OnBnClickedCheckStation2()
{
	UpdateData(TRUE);
	GetDlgItem(IDC_BUTTON_SENDER2)->EnableWindow(m_bStation[1]);
}
void CSearchEqView::OnBnClickedCheckStation3()
{
	UpdateData(TRUE);
	GetDlgItem(IDC_BUTTON_SENDER3)->EnableWindow(m_bStation[2]);
}
void CSearchEqView::OnBnClickedCheckStation4()
{
	UpdateData(TRUE);
