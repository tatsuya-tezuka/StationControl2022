...
bool CShareAlarmIf::Create(bool bServer, int nStation)
{
	...
	if (mServer == true){
		...
		if (GetLastError() == ERROR_ALREADY_EXISTS){
			...
			CLogTraceEx::Write(_T(""), _T("CShareAlarmIf"), _T("Create"), (LPCTSTR)strShareMemory, _T("Server is true: m_hMap already exists."), nLogEx::info);
			if (m_hMap != NULL)
			{
				::CloseHandle(m_hMap);
				m_hMap = NULL;
		...
		if (m_hMap == NULL || GetLastError() == ERROR_FILE_NOT_FOUND)
		{
			...
			CLogTraceEx::Write(_T(""), _T("CShareAlarmIf"), _T("Create"), (LPCTSTR)strShareMemory, _T("Server is false: m_hMap dose not exist."), nLogEx::info);
			if (m_hMap != NULL)
			{
				::CloseHandle(m_hMap);
				m_hMap = NULL;
	...
	CLogTraceEx::Write(_T(""), _T("CShareAlarmIf"), _T("Create"), (LPCTSTR)strShareMemory, _T("m_hMap's activate success."), nLogEx::info);
...
int CShareAlarmIf::ConvertAlarmData(CString strFileName, vector<stAlarmItem>& list)
{
	...
	TRY
	{
		...
		// 更新日時
		sprintf_s(data.updatetime, mAlarmItemTimeMax, "%s", (LPCSTR)CStringA(str));
		...
		while (!feof(fp) && list.size() < mAlarmItemMax){
			...
			while (AfxExtractSubString(str, cbuf, i++, ',')){
				...
				case	0:
					// 局名
					data.line.no = (DWORD)(list.size() + 1);
