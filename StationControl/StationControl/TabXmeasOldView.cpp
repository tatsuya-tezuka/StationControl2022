...
IMPLEMENT_DYNCREATE(CTabXmeasOldView, CDialogEx)

CTabXmeasOldView::CTabXmeasOldView(CWnd* pParent /*=NULL*/)
	: CDialogEx(CTabXmeasOldView::IDD, pParent)
{

}

CTabXmeasOldView::~CTabXmeasOldView()
{
}

void CTabXmeasOldView::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CTabXmeasOldView, CDialogEx)
END_MESSAGE_MAP()


// CTabXmeasOldView メッセージ ハンドラー

BOOL CTabXmeasOldView::OnInitDialog()
{
...
*/
/*============================================================================*/
void CTabXmeasOldView::UpdateStatus()
{
	CEngValCnv::obsname_t *obs_adr;	// 監視項目アドレス
	CSatelliteData& sd = theApp.GetSatelliteData();

	int szControl = sizeof(mXmeasOldParam) / sizeof(mXmeasOldParam[0]);
	for (int i = 0; i < szControl; i++)
	{
		CString mControlString = mXmeasOldParam[i].nameK;
		CString str;
		stControlValue val;
		val.type = mXmeasOldParam[i].type;
		...
		case	eControlNumeric:
			...
			break;
		case	eControlDouble:
			if (mXmeasOldParam[i].id == IDC_STATIC_RNGNUMVAL){
			...
			else if (mXmeasOldParam[i].id == IDC_STATIC_INTGT1VAL || mXmeasOldParam[i].id == IDC_STATIC_INTGT2VAL){
				str = CSatelliteData::AddCanma(obs_adr->d_data, 0);
			}
			else
		...
		((CColorStatic*)GetDlgItem(mXmeasOldParam[i].id))->SetText(str);
...
*/
/*============================================================================*/
void CTabXmeasOldView::InitStatus()
{
	int szControl = sizeof(mXmeasOldParam) / sizeof(mXmeasOldParam[0]);
	for (int i = 0; i < szControl; i++)
	{
...
LRESULT CTabXmeasOldView::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message){
