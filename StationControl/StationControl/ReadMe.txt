================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : StationControl プロジェクトの概要
===============================================================================

この StationControl アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、StationControl アプリケーションを構成する各ファイルの内容の概要が含まれています。

StationControl.vcxproj
...
StationControl.vcxproj.filters
...
StationControl.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CStationControlApp アプリケーション クラスを宣言します。

StationControl.cpp
    これはメインのアプリケーション ソース ファイルで、CStationControlApp アプリケーション クラスが含まれています。
...
res\StationControl.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル StationControl.rc にインクルードされます。

res\StationControl.rc2
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、StationControl.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////
...
StationControlDoc.h, StationControlDoc.cpp - ドキュメント
    これらのファイルには、CStationControlDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (CStationControlDoc::Serialize を使用)。

StationControlView.h, StationControlView.cpp - ドキュメントのビュー
    これらのファイルには、CStationControlView クラスが含まれます。
    CStationControlView オブジェクトは CStationControlDoc オブジェクトを表示するために使用されます。

res\StationControlDoc.ico
    これは、CStationControlDoc クラスの MDI 子ウィンドウのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル StationControl.rc にインクルードされます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、StationControl.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

StationControl.manifest
