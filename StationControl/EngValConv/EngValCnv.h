...
//class __declspec(dllexport) CEngValCnv
class AFX_EXT_CLASS CEngValCnv
{
public:
	CEngValCnv(char* db_file_name);
	CEngValCnv();
	~CEngValCnv();
...
public:
	...
	typedef struct {  /*「情報フィールド＋付加情報＋テレメトリフレーム」 */
		TL_INFO		info;		// 情報フィールド
		TL_ADDINFO	addinfo;	// 付加情報部
		TL_RECDATA	recvdata;	// テレメトリ受信データ（可変）
	} TL_TLMDATA;
	...
	*******************************************************************************
	*/
	struct timecvt_tbl_t {	/* 時刻変換情報定義テーブル */
	...
	struct  sdtp_msg_ex_t {               /* sdtp メッセージテーブル 2017年版 */
		unsigned char uca_SdtpDummy[26];/* 受信側メッセージキューＩＤ */
		unsigned char uca_SdtpPDULen[4];/* PDU長 */
		unsigned char uca_SdtpRecNum[2];/* レコード数 */
		unsigned char uca_buff[MAX_SDTP];/* データバッファ */
...
public:
	...
	CString		mDbName;
	map< CString, vector<obs_tbl_res_t> >	mOutDataList;			// 工学値変換用監視名マップ
	vector<CString>							mMonDataFileList;		// 工学値変換対象のファイルリスト
	bool									mCheckOutDataList;

protected:
	std::vector<obs_tbl_res_t> data_list;
	...
	struct obs_tbl_t obs_tbl[OBS_MAX];

	long m_lastCnvAfterType;

	map<int, vector<obs_tbl_t>> obs_map;
...
public:
	...
	bool GetMonitorData(TL_TIME& timeTarget, map< CString, vector<obs_tbl_res_t> >& outDataList);
	bool GetMonitorDataEx(TL_TIME* timeTarget);
	void ClearMonitorName();
	void AddMonitorName(TCHAR* name);
	map< CString, vector<obs_tbl_res_t> >&	GetOutDataList() { return mOutDataList; }
	vector<CString>&						GetMonDataFileList() { return mMonDataFileList; }
	bool GetCheckCheckOutDataList() { return mCheckOutDataList; }
	void SetCheckCheckOutDataList(bool val) { mCheckOutDataList = val; }
...
protected:
	bool ConvertDirectRangeTime(TL_TIME& dtime, TL_TIME& rtime, bool timezero=true);
	bool ConvertStatusChar(char* stsname, int len);
	CStringA UTF8_3CharsToSJis(char *pUtf8Str);
	CStringA EUCToSJis(char *pEucStr);
	bool IsObsName(char* obsname, map< CString, vector<obs_tbl_res_t> >& outDataList);
...
public:
	int  CalcFromFileEx(char *cp_loadadr_p, map< CString, vector<obs_tbl_res_t> >& outDataList);
	int  CalcFromFile(char *cp_loadadr_p);
	int  CalcFromMsg(sdtp_msg_ex_t* msg);
	//vector<obs_tbl_res_t>& GetAllVal() { return data_list; }
	vector<obs_tbl_res_t>& GetAllVal() { return data_list; }

	long GetCnvResType()	{ return m_lastCnvAfterType; }
	static BOOL IsCnvResLong(struct obsname_t& obs_adr)
	{
	...
	static BOOL IsCnvResDouble(struct obsname_t& obs_adr)
	{
	...
	static BOOL IsCnvResLongDouble(struct obsname_t& obs_adr)
	{
	...
	static BOOL IsCnvResStatusStr(struct obsname_t& obs_adr)
	{
	...
	static BOOL IsCnvResTimeStr(struct obsname_t& obs_adr)
	{
...
protected:
	...
	int ud_ext_adr(char sz_watch[], obsname_t *obs_adr, ext_tbl_t *ext_adr[]);
	...
	int ud_obsname_adr(char sz_watch[], char *shmadr, struct obsname_t *obs_adr[]);
	int CreateObsTbl();
