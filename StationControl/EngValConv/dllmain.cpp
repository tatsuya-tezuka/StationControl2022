...
static AFX_EXTENSION_MODULE EngValConvDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("EngValConv.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(EngValConvDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(EngValConvDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("EngValConv.DLL Terminating!\n");

		// デストラクターが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(EngValConvDLL);
	}
	return 1;   // OK
}
