...
CEngValCnv::CEngValCnv(char* db_file_name)
{
	db_data = NULL;
	db_data2 = NULL;
	try{
	...
	mCheckOutDataList = false;
}

CEngValCnv::CEngValCnv()
{
	db_data = NULL;
	db_data2 = NULL;
...
*/
/*============================================================================*/
void CEngValCnv::SetEqMonDbFileName(TCHAR* val)
{
	mDbName = CString(val);
...
*/
/*============================================================================*/
int CEngValCnv::CreateObsTbl()
{
	struct obsname_t *obs_adrs;
	struct ext_tbl_t *ext_adr;
	char			 *mon_adr;
	long			l_rc;
	...
	try{
		...
		int t = 0;
		while (1){
			obs_adrs = (struct obsname_t *)mon_adr;
...
*/
/*============================================================================*/
void CEngValCnv::SetMonDataPath(TCHAR* val)
{
...
@retval	bool
  ...
  */
/*============================================================================*/
bool CEngValCnv::GetMonitorData(TL_TIME& timeTarget, map< CString, vector<obs_tbl_res_t> >& outDataList)
{
	// 最小タイマ分解能を設定(1ms)
	timeBeginPeriod(1);
	...
	for (itr = datafilelist.begin(); itr != datafilelist.end(); itr++){
		...
		vector<CEngValCnv::obs_tbl_res_t>& data = GetAllVal();
		vector<CEngValCnv::obs_tbl_res_t>::iterator itrd;
		for (itrd = data.begin(); itrd != data.end(); itrd++){
			...
			map< CString, vector<obs_tbl_res_t> >::iterator itrn;
...
bool CEngValCnv::GetMonitorDataEx(TL_TIME* timeTarget)
{
	// 最小タイマ分解能を設定(1ms)
	timeBeginPeriod(1);
	DWORD	dwStart = timeGetTime();
...
void CEngValCnv::ClearDataFileList()
{
	mMonDataFileList.clear();
}

void CEngValCnv::AddDataFileList(TCHAR* val)
{
	TCHAR buf[_MAX_PATH];
...
*/
/*============================================================================*/
bool CEngValCnv::ConvertStatusChar(char* stsname, int len)
{
	char src[64];
	char dest[64];
	...
	const char mon[32] = {
		0x56, 0x4C, 0x42, 0x49, 0xB1, 0xBF, 0xCD, 0xD1,
		0xBB, 0xFE, 0xB4, 0xD6, 0x20, 0x20, 0x20, 0x20,
...
*/
/*============================================================================*/
bool CEngValCnv::ConvertDirectRangeTime(TL_TIME& dtime, TL_TIME& rtime, bool timezero/*=true*/)
{
...
*/
/*============================================================================*/
void CEngValCnv::GetTimeRangePath(time_t sttime, time_t sptime, vector<CString>& fileList)
{
	fileList.clear();
...
*/
/*============================================================================*/
void CEngValCnv::GetDataFileList(CString& path, vector<CString>& outFileList)
{
	// 対象フォルダ内のファイル情報を取得する
...
*/
/*============================================================================*/
void CEngValCnv::ClearMonitorName()
{
	mOutDataList.clear();
}
...
*/
/*============================================================================*/
void CEngValCnv::AddMonitorName(TCHAR* name)
{
	CString tag(name);
	vector<obs_tbl_res_t> val;
	map< CString, vector<obs_tbl_res_t> >::iterator itr = mOutDataList.find(tag);
	if (itr == mOutDataList.end())
		mOutDataList.insert(map< CString, vector<obs_tbl_res_t> >::value_type(tag, val));
}
/*============================================================================*/
/*! 監視データアクセス
...
*/
/*============================================================================*/
bool CEngValCnv::IsObsName(char* obsname, map< CString, vector<obs_tbl_res_t> >& outDataList)
{
	if (outDataList.size() == 0){
		if (mCheckOutDataList == false)
			return true;
		else
			return false;
	}
...
*/
/*============================================================================*/
CStringA CEngValCnv::UTF8_3CharsToSJis(char *pUtf8Str)
{
	wchar_t pUtfMsg[4];
...
*/
/*============================================================================*/
CStringA CEngValCnv::EUCToSJis(char *pEucStr)
{
	// EUCの２バイト文字かのチェック
...
int CEngValCnv::DbRead(char* db_file_nam, char *cp_loadadr_p)
{
	int errno;
	struct stat stbuf;
	int    i_readbyte;
...
　　◆注意◆呼び出しもとで戻り値のTL_TLMDATA*アドレスを削除すること
  ...
  */
/*============================================================================*/
void* CEngValCnv::MonRead(FILE **fp)
{
	size_t i_readbyte2;
	...
	i_readbyte2 = fread(&info, sizeof(TL_INFO), 1, *fp);
	if (i_readbyte2 <= 0){
		return NULL;
	}
	...
	pdata = (TL_TLMDATA*)::malloc(sizeof(TL_INFO) + info.l_reclen);
	if (pdata == NULL)
		return NULL;

	memcpy(&(pdata->info), &info, sizeof(TL_INFO));
...
*/
/*============================================================================*/
int CEngValCnv::CalcFromFileEx(char* nama_data_file, map< CString, vector<obs_tbl_res_t> >& outDataList)
{
	struct obsname_t *obs_adrs;
	struct ext_tbl_t *ext_adr;
	char      *mon_adr;
	...
	std::vector<CEngValCnv::obs_tbl_res_t>::iterator itr;
			...
			for (l = 0; obs_tbl[l].obs_adr != NULL; l++) {
				...
				if (l_dataid == obs_tbl[l].l_dataid || obs_tbl[l].l_dataid == -1) {
					...
					if (l_rc == 0 && l_inflg == 1) {
						...
						res.l_dataid = obs_tbl[l].l_dataid;
						memcpy_s(&res.obs_adr, sizeof(obsname_t), obs_tbl[l].obs_adr, sizeof(obsname_t));
						...
						if (bFirst == true){
							...
							vector<obs_tbl_res_t>::iterator itr;
...
*/
/*============================================================================*/
int CEngValCnv::CalcFromFile(char* nama_data_file)
{
	struct obsname_t *obs_adrs;
	struct ext_tbl_t *ext_adr;
	char      *mon_adr;
...
int CEngValCnv::CalcFromMsg(sdtp_msg_ex_t* msg)
{
	char      *mon_adr;
	long      l_dsu;
...
//			long double time_val = 0;
			...
			for (int i = 0; i < l_dsu; i++) {
				...
				vector<obs_tbl_t> &list = obs_map[l_dataid];
				for (int l = 0; l < (int)list.size(); ++l) {
					...
					if (l_dataid == st.l_dataid || st.l_dataid == -1) {
						...
						if (l_rc == 0 && l_inflg == 1) {
							...
							if (CStringA(res.obs_adr.sz_obsname) == CStringA("CTRL.EL_AT_MAX_EL")){
								int u = 0;
							}
							if (CStringA(res.obs_adr.sz_obsname) == CStringA("CTRL.PASS_NO")){
								int u = 0;
							}
							if (CStringA(res.obs_adr.sz_obsname) == CStringA("CTRL.SAT_NAME")){
								int u = 0;
							}
							if (CStringA(res.obs_adr.sz_obsname) == CStringA("CTRL.PRED_FILE")){
								int u = 0;
							}

							// 対象監視項目名が存在するかのチェック
//							TRACE("###OBSNAME:%s \n", res.obs_adr.sz_obsname);
							if (IsObsName(res.obs_adr.sz_obsname, mOutDataList) == true){
								// 変換結果ステータス文字列のコード変更（UTF-8->SJIS EUC-JI->SJIS）
								if (mOutDataList.size() == 0)	// setlocalで落ちるため
									ConvertStatusChar(res.obs_adr.sz_sts_name, 64);
...
//								TRACE("--- CLEAR\n", res.obs_adr.sz_obsname);
				...
				//long double time_val = 0;
				//for (itr = data_list.begin(); itr != data_list.end(); itr++){
				//	CEngValCnv::obs_tbl_res_t d = *itr;
				//	if (d.l_grp_cnt == cnt){
...
int CEngValCnv::ud_montbl_data64(char    *shmadr, int l_dataid, unsigned char ca_data[], int l_offset,
	...
	long double *ldp_data, long *lp_bitinfo)
{
	...
	struct obsname_t *obs_ex_adr;  /* 監視項目定義情報 */
	struct if_tbl_t  *if_adr;      /* 条件定義情報 */
	struct cal_tbl_t *cal_adr;     /* 他項目計算情報 */
	struct ext_tbl_t *ext_adr;     /* 各抽出定義情報 */
	struct pol_tbl_t *pol_adr;     /* 方程式定義定義テーブル */
	struct carv_tbl_t *carv_adr;   /* 校正点カーブ定義テーブル */
	struct timecvt_tbl_t *time_adr; /* 時刻変換定義テーブル */
	...
	//設備監視名称情報 from db
	obs_adr = (struct obsname_t *)(shmadr + l_offset);
	...
	*lp_bitinfo = 0;  /* 整数ビット数:整数抽出データのビット長 */
	...
	while (1) {
		...
		case ELSEIF_ID:
			...
			if_adr = (struct if_tbl_t  *)mon_adr;
			...
			if (ds_htonl(if_adr->l_offset) != -1) {
				...
				if (l_rc != 0 || l_if_inflg == 0) {
					obs_ex_adr = (struct obsname_t *)(shmadr + if_adr->l_offset);
		...
		case CAL_ID:
			if (*l_inflg == 1) {
				cal_adr = (struct cal_tbl_t  *)mon_adr;
				...
				if (ds_htonl(cal_adr->l_offset) != -1) {
					...
					if (l_rc != 0 || l_if_inflg == 0) {
						obs_ex_adr = (struct obsname_t *)(shmadr + cal_adr->l_offset);
		...
		case EXTBCDS_ID:
			...
			*d_data += (d_dataf[0] * pow(10.0, d_dataf[1]));
			...
			/*-----------------------------------------------------------*/
		case POL_ID:
			pol_adr = (struct pol_tbl_t *)mon_adr;
			...
			*d_data = 0.0;
			...
			/*-----------------------------------------------------------*/
		case CARVTBL_ID:
			carv_adr = (struct carv_tbl_t *)mon_adr;
			...
			/*-----------------------------------------------------------*/
		case TIMECVT_ID:
			time_adr = (struct timecvt_tbl_t *)mon_adr;
			*l_grmd = 2;
			m_lastCnvAfterType |= 1 << eCnvResType_TimeStr;
...
void CEngValCnv::ua_ldbitfieldll(unsigned long long *pull_outbuf, unsigned char inputbuf[]
...
void CEngValCnv::ua_ldbitfield(unsigned long *pul_outbuf, unsigned char inputbuf[]
	, long l_wordno, unsigned long ul_bitno, unsigned long ul_bitlen, long l_signflg)
{
	unsigned long ul_w;
	//unsigned char *p;
...
void CEngValCnv::ua_stbitfield(unsigned long ul_input, unsigned char outputbuf[],
...
void CEngValCnv::ua_insbitfield(unsigned char *tgtadr, long l_inspos, long l_inslen, char  *suadr, long l_extpos)
{
...
int CEngValCnv::ud_montbl_data(char *shmadr, int l_dataid, unsigned char ca_data[], int l_offset, int *l_inflg, int *l_grmd, double *d_data, char sz_sts_name[])
{
	long double ld_data;
	long        l_bitinfo;
...
void CEngValCnv::ub_bcdext(double *d_data, unsigned char ca_data[], long l_word, long l_bit, long l_len)
{
	long   i;
	long   l;
	long   m;
...
void CEngValCnv::ub_dtime_to_asc(double d_uttime, char *sz_time)
{
	long l_100usec;
...
//char    *cap_ss;       /* i: 時刻(ss)  */
//
double CEngValCnv::ua_ddd_to_dtime(char *cap_ddd, char *cap_hh, char *cap_mm, char *cap_ss)
{
...
void CEngValCnv::ub_bcd6_to_bcd(unsigned char achBCD6[], unsigned char achYYYY[], unsigned char achTime[])
{
	//long l_time;
	long long l_time;
	struct tm p_tm;
...
double CEngValCnv::ub_bcd_to_dtime(unsigned char uca_yyyy[], unsigned char uca_time[])  /*  0 1 */
{
...
double CEngValCnv::ub_asc_to_dtime(char *sz_time)
{
	int i;
	struct tm t;
	double d_sec, d_0sec;
...
int CEngValCnv::ud_ext_adr(char sz_watch[], obsname_t *obs_adr, ext_tbl_t *ext_adr[])
{
	char *mon_adr;
	/*----- 共有メモリのアドレスＧＥＴ -----*/
	...
	mon_adr = (char *)obs_adr;
	while (1) {
		ext_adr[0] = (struct ext_tbl_t *)mon_adr;
...
double CEngValCnv::dswap(double d)
{
#if IS_BIG_ENDIA
	double s = d;
...
double CEngValCnv::dswap2(double d)
{
	double s = d;
	char *ps((char*)&s), *pd((char*)&d);
...
float CEngValCnv::fswap(float d)
{
	float s = d;
	char *ps((char*)&s), *pd((char*)&d);
...
long CEngValCnv::ds_htonl(long val)
{
#if IS_BIG_ENDIA
	return htonl(val);
#else
...
int CEngValCnv::uc_valchk(char *s, int *i_p, double *d_p, int l_opt)
{
	int     i = 2;                            /* 文字列カウンタ */
	int     j = 0;                            /* カウンタ */
...
int CEngValCnv::uc_valchkll(char *s, long *i_p, long double *d_p, long l_opt)
{
	int     i = 2;                            /* 文字列カウンタ */
	int     j = 0;                            /* カウンタ */
	char    sz_dmy[512];                    /* sscanfダミーバッファ */
...
int CEngValCnv::ud_sts_flg(char sz_watch[], char *sz_path, char *shmadr, long l_n, int *l_grmd, long *l_offset, char *sz_sts_tbl[])
{
	struct obssts_tbl_t *obssts_adr;/* ステータス情報テーブル */
	struct obsname_t *obs_adr; /* 監視項目定義情報 */
	struct ext_tbl_t *ext_adr; /* 各抽出定義情報 */
...
int CEngValCnv::ud_obsname_adr(char sz_watch[], char *shmadr, struct obsname_t *obs_adr[])
{
	char *mon_adr;
