================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : OperationTimeLine プロジェクトの概要
===============================================================================

この OperationTimeLine アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、OperationTimeLine アプリケーションを構成する各ファイルの内容の概要が含まれています。

OperationTimeLine.vcxproj
...
OperationTimeLine.vcxproj.filters
...
OperationTimeLine.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、COperationTimeLineApp アプリケーション クラスを宣言します。

OperationTimeLine.cpp
    これはメインのアプリケーション ソース ファイルで、COperationTimeLineApp アプリケーション クラスが含まれています。

OperationTimeLine.rc
...
res\OperationTimeLine.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル OperationTimeLine.rc にインクルードされます。

res\OperationTimeLine.rc2
...
res\Toolbar.bmp
    ...
    初期ツール バーと初期ステータス バーは、CMainFrame クラスで生成されます。ツール バーのボタンを追加するには、リソース エディターを使用してこのツール バーのビットマップを編集し、OperationTimeLine.rc 内の IDR_MAINFRAME TOOLBAR 配列を更新します。
/////////////////////////////////////////////////////////////////////////////
...
OperationTimeLineDoc.h, OperationTimeLineDoc.cpp - ドキュメント
    これらのファイルには、COperationTimeLineDoc クラスが含まれます。これらのファイルを編集して、特殊なドキュメント データを追加し、ファイルの保存と読み込みを実装することができます (COperationTimeLineDoc::Serialize を使用)。

OperationTimeLineView.h, OperationTimeLineView.cpp - ドキュメントのビュー
    これらのファイルには、COperationTimeLineView クラスが含まれます。
    COperationTimeLineView オブジェクトは COperationTimeLineDoc オブジェクトを表示するために使用されます。

res\OperationTimeLineDoc.ico
    これは、COperationTimeLineDoc クラスの MDI 子ウィンドウのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル OperationTimeLine.rc にインクルードされます。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、OperationTimeLine.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

OperationTimeLine.manifest
