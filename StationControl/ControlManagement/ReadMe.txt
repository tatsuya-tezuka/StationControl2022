================================================================================
    MFC (MICROSOFT FOUNDATION CLASS) ライブラリ : ControlManagement プロジェクトの概要
===============================================================================

この ControlManagement アプリケーションは、アプリケーション ウィザードによって作成されました。このアプリケーションは MFC の基本的な使い方を示すだけでなく、アプリケーションを作成するための開始点でもあります。

このファイルには、ControlManagement アプリケーションを構成する各ファイルの内容の概要が含まれています。

ControlManagement.vcxproj
...
ControlManagement.vcxproj.filters
...
ControlManagement.h
    ...
    その他のプロジェクト固有のヘッダー (Resource.h など) が含まれ、CControlManagementApp アプリケーション クラスを宣言します。

ControlManagement.cpp
    これはメインのアプリケーション ソース ファイルで、CControlManagementApp アプリケーション クラスが含まれています。

ControlManagement.rc
...
res\ControlManagement.ico
    これはアプリケーションのアイコンとして使用されるアイコン ファイルです。このアイコンは、メイン リソース ファイル ControlManagement.rc にインクルードされます。

res\ControlManagement.rc2
...
ControlManagementDlg.h, ControlManagementDlg.cpp - ダイアログ
    これらのファイルには、CControlManagementDlg クラスが含まれます。このクラスはアプリケーションのメイン ダイアログの動作を定義します。ダイアログのテンプレートは、Microsoft Visual C++ で編集できる ControlManagement.rc に含まれています。
...
StdAfx.h, StdAfx.cpp
    これらのファイルは、ControlManagement.pch という名前のプリコンパイル済みヘッダー (PCH) ファイルと、StdAfx.obj という名前のプリコンパイル済みの型ファイルをビルドするために使用されます。

Resource.h
    これは、新しいリソース ID を定義する標準のヘッダー ファイルです。このファイルの読み込みおよび更新は、Microsoft Visual C++ で行います。

ControlManagement.manifest
